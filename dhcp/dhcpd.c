...
int main(int argc, char *argv[]){
    ...
    char sendBuf[MAX_SIZE]; /* Buffer for send string */
    ...
    while(1){
        ...
        if ((state = get_option(&packet, DHCP_MESSAGE_TYPE)) == NULL) {
            printf("couldn't get option from packet, ignoring\n");
            continue;
	}
        ...
        switch(state[0]){
            ...
            case DHCPDISCOVER: {
                ...
                if(availabelIPNumber("dhcp.config") <= 0){
                    ...
                    memcpy(&message.options[0], &messageType, sizeof(u_int8_t));
            ...
            // DHCP Release (Retrive the IP address without sending the ACK)
            case 0x07:{ 
            ...
            // DHCP Inform (Response ACK without Option 51 IP address Lease Time)
            case 0x08: {
