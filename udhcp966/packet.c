...
/* 读取·packet，read a packet from socket fd, return -1 on read error, -2 on packet error */
int get_packet(struct dhcpMessage *packet, int fd)
{
	...
	DEBUG(LOG_INFO, "Received a packet");
        ...
        /*vender字段判断是否强制设定flag字段为1（广播）会传递到发送包的*/
	if (packet->op == BOOTREQUEST && (vendor = get_option(packet, DHCP_VENDOR))) {//逻辑与，前后条件同时满足表达式为真
		   ...
		   u_int32_t dest_ip, int dest_port, unsigned char *dest_arp, int ifindex)
{
	...
	dest.sll_ifindex = ifindex; //server端口指定的网卡
	dest.sll_halen = 6;
